...
void search (char *text , char *pat)
{
    ...
    int s,j;
    for(s=0;s<=n-m;s++)
    {             // Iteration over n-m+1 values of shift
        for(j=0;j<m;j++)
        {
...
int main()
{
    ...
    if(myfile.is_open())
    {
        while(getline(myfile,line))
        {
            ...
            search(a,"The stranger followed in their tracks, sheathing his sword and dagger. As they passed the tables the remaining customers hid their faces from the dangerous stranger");
        }
    }
